<script lang="ts">
    ...
    let folder: string = '';
    let fileName: string = '';
    let githubURL: string = ''; // Declare githubURL here

    const pattern: RegExp = /-spn\d+-/; // -spn{order}- is the pattern for series post

    if (!pattern.test($page.params.slug)) {
        // not a series post
        githubURL = `https://github.com/${githubConfig.username}/${githubConfig.repo}/blob/${githubConfig.branch}/posts/${$page.params.slug}/page.md`;
    } else {
