import { k as derived, w as writable, l as readonly, r as readable } from "./exports.js";
import { j as get_store_value, k as getContext, s as setContext, o as onDestroy } from "./ssr.js";
...
function createCommand(props) {
  ...
  function sort(state2, shouldFilterVal) {
    ...
    getValidItems(rootEl).sort((a, b) => {
      ...
      if (isHTMLElement$2(group)) {
        if (item.parentElement === group) {
          group.appendChild(item);
        } else {
          if (!isHTMLElement$2(closest))
            return;
          group.appendChild(closest);
        }
      } else {
        if (!isHTMLElement$2(list))
          return;
  ...
  function getValidItems(rootElement) {
    ...
    if (!rootEl)
      return [];
    return Array.from(rootEl.querySelectorAll(VALID_ITEM_SELECTOR)).filter((el) => isHTMLElement$2(el));
  }
...
function effect(stores2, fn) {
  const unsub = derivedWithUnsubscribe(stores2, (stores3, onUnsubscribe) => {
    return {
      stores: stores3,
      onUnsubscribe
    };
...
export {
  ...
  useEscapeKeydown as j,
  useClickOutside as k,
  isBrowser as l,
  addMeltEventListener as m,
  noop$1 as n,
  kbd as o,
  ...
  handleEscapeKeydown as v,
  noop as w,
  getCtx as x,
  isHTMLElement$2 as y,
  getState as z
};
